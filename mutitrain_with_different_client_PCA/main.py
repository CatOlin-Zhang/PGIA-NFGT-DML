...
def main():
    ...
    datasets = {}
    client_initial_data_sizes = {}  # 统计每个客户端的初始数据量
    for client_id, indices in clients.items():
        ...
        client_initial_data_sizes[client_id] = len(indices)

    # 初始化累计数据量字典
    client_cumulative_data_sizes = {client_id: 0 for client_id in client_ids}
    client_dropped_samples_count = {client_id: 0 for client_id in client_ids}  # 记录每个客户端丢弃的样本数量
    ...
    # 每轮更新参与的客户端数量
    clients_per_epoch = [5, 4, 3, 4, 2] * (epochs // 5)  # 示例数组，重复多次以覆盖所有epoch

    epoch_losses = []  # 存储每轮的平均损失值
    client_selection_count = {client_id: 0 for client_id in client_ids}  # 统计每个客户端被选中的次数
    ...
    for epoch in range(epochs):
        ...
        for client_id in selected_clients:
            ...
            local_model, client_avg_loss, dropped_samples_count = batch_gradient_descent_federated(
            ...
            # 更新丢弃的样本数量
            client_dropped_samples_count[client_id] += dropped_samples_count

        # 更新未被选中客户端的累计数据量
        for client_id in client_ids:
            if client_id not in selected_clients:
                client_cumulative_data_sizes[client_id] += client_initial_data_sizes[client_id]
    ...
    for client_id, count in client_selection_count.items():
        print(f'{client_id}: {count}')

    # 打印每个客户端的累计数据量
    print("\nClient Cumulative Data Sizes:")
    for client_id, size in client_cumulative_data_sizes.items():
        print(f'{client_id}: {size}')

    # 打印每个客户端丢弃数据的比例
    print("\nClient Drop Ratios:")
    for client_id in client_ids:
        initial_data_size = client_initial_data_sizes[client_id]
        cumulative_data_size = client_cumulative_data_sizes[client_id]
        dropped_samples = client_dropped_samples_count[client_id]
        drop_ratio = dropped_samples / cumulative_data_size if cumulative_data_size > 0 else 0
        print(f'{client_id}: {drop_ratio:.4f}')
