...
tolerance = 1e-6; % 损失函数变化的容忍度
m = size(X_b, 1); % 样本数量

% 初始化参数实际上决定了从哪一点开始，后续将从同一个点开始
...
% 第一次SGD:
% 目标：找到最大迭代次数，若没有则为提前设置的最大值。
% 迭代次数：最大迭代次数 max_iterations。
% 梯度阈值：无限制。
% 收敛条件：损失函数的变化小于 tolerance (1e-6)。
% 第二次SGD:
% 目标：使用第一次找到的最大迭代次数的0.8倍。
% 迭代次数：convergence_iteration * 0.8。
% 梯度阈值：无限制。
% 收敛条件：损失函数的变化小于 tolerance (1e-6)。
% 第三次SGD:
% 目标：使用与第二次相同的迭代次数，但增加梯度阈值以过滤小梯度更新。
% 迭代次数：convergence_iteration * 0.8。
% 梯度阈值：所维护的梯度数组的前75%。
% 收敛条件：损失函数的变化小于 tolerance (1e-6)。
% 第四次SGD:
% 目标：使用与第二次相同的迭代次数，但使用更大的梯度阈值。
% 迭代次数：convergence_iteration * 0.8。
% 梯度阈值：所维护的梯度数组的前50%。
% 收敛条件：损失函数的变化小于 tolerance (1e-6)。

% 实验1与2表明，只是通过限制次数不会影响同一次数下（未达到收敛极限前）的收敛效果 ？
% --》联合实验3或4：过滤更新过程中造成的丢失造成的负面影响小于因为只传递重要梯度的优势影响。
% 
% 实验2与3表明，同一次数下进行一定程度的过滤，不仅不会影响收敛效果，反而会加快收敛
% 
% 实验3与4表明，在实验3条件下，设置更加苛刻的有效更新条件，会使得收敛更加快速 （但具有平台效应？）
% 方案3与4本身节约的是通信资源，大量使用本地计算资源，理论上p的值越小收敛曲线相比于原本越明显。
% (过低p值有可能导致本地无法更新结果)
