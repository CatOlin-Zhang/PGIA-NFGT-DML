...
%主成分分析协方差方案需要高区分度拟合函数情形才能发挥更高的效果
...
% 计算协方差矩阵及其特征值和特征向量
% 协方差矩阵参考 
% https://blog.csdn.net/herr_kun/article/details/80345096?utm_medium=distribute.pc_relevant.none-task-blog-2~default~baidujs_baidulandingword~default-1-80345096-blog-120182181.235^v43^pc_blog_bottom_relevance_base7&spm=1001.2101.3001.4242.2&utm_relevant_index=3
...
% 影响成分所解释方差，PCA坐标变换后新图像将包含原图像多少的信息量，越高越精准但优化效果会降低。
% 原理参考 https://blog.csdn.net/MoreAction_/article/details/107463336
% 算法参考 https://blog.csdn.net/MoreAction_/article/details/107318158
...
while iteration_count < new_max_iterations
    ...
    projection_history=[projection_history,projection];%访问对应记录数组，可以选取合适的用于更新的阈值。
    % 判断是否用于模型更新
    if any(abs(projection) > 2.5) 
    % 阈值越大说明两数据相关度越大，越高收敛性能越好但代价是最后收敛计算时间长
    % 初步测试1.5左右与第三次实验齐平累计能量0.65，2.0左右与第四次实验齐平累计能量0.65，2.5超过第四次实验累计能量0.65
        gradient = 2 * xi' * (xi * theta - yi);
        theta = theta - eta * gradient;
        iteration_count = iteration_count + 1;
...
figure;
plot(loss_history, 'b', 'DisplayName', 'Baseline');
hold on;
plot(loss_history_new, 'r', 'DisplayName', '第二次限定迭代次数SGD结果');
plot(loss_history_filtered, 'g', 'DisplayName', '按模长丢失(前75%)');
plot(loss_history_filtered_4th, 'm', 'DisplayName', '按模长丢失(前50%)');
plot(loss_history_filtered_5th, 'c', 'DisplayName', 'PGIA');
xlabel('Iteration');
ylabel('Loss');
title('SGD损失函数历史');
legend show;
