...
class FedAvgAPI(object):
    ...
    def _client_sampling_average(self, round_idx, client_num_in_total, client_num_per_round, group_to_client_indexes):
        ...
        for i in range(self.args.group_num):
            for j in range(self.args.client_num_per_round//self.args.group_num):
    ...
    def _aggregate(self, w_locals):
        """安全的联邦平均聚合方法"""
        if not w_locals:
            raise ValueError("Empty w_locals list: no client updates to aggregate!")
        ...
        for item in w_locals:
            if item is None:  # 跳过无效更新
                continue
            sample_num, params = item
            if sample_num <= 0:  # 检查样本数有效性
                continue
        ...
        if training_num == 0:
            # 返回第一个客户端的参数（或随机初始化）
            if valid_updates:
                return valid_updates[0][1].copy()
            else:
                return w_locals[0][1].copy() if w_locals else None

        # 3. 加权聚合（优化后的实现）
        _, averaged_params = valid_updates[0] if valid_updates else w_locals[0]

        # 初始化累加器
        for k in averaged_params.keys():
            averaged_params[k] = torch.zeros_like(averaged_params[k])

        # 累加加权参数
        for sample_num, local_params in valid_updates:
    ...
    def _local_test_on_all_clients(self, round_idx):
        ...
        logging.info(stats)
        with open("train_acc.txt", 'a', encoding="utf-8") as f:
            f.write(str(train_acc))
            f.write("\n")
        with open("test_acc.txt", 'a', encoding="utf-8") as f:
