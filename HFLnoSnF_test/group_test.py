...
class Group(FedAvgAPI):
    ...
    def train(self, global_round_idx, w, sampled_client_indexes):
        sampled_client_list = [self.client_dict[client_idx] for client_idx in sampled_client_indexes]  # {list:4}[993, 859, 553, 672]  # 4个客户端
        w_group = w  # weight+bias
        w_group_list = []
        for group_round_idx in range(self.args.group_comm_round):
            logging.info("Group ID : {} / Group Communication Round : {}".format(self.idx, group_round_idx))  # 该组内的迭代
            w_locals_dict = {}
