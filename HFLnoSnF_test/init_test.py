...
import fedml
from collect_env_test import collect_env
...
def init(args=None):
    if args is None:
        args = arguments_test.load_arguments(fedml._global_training_type, fedml._global_comm_backend)

    """Initialize FedML Engine."""
    collect_env(args)

    fedml._global_training_type = args.training_type  # 'simulation'
    fedml._global_comm_backend = args.backend  # 'sp'
    ...
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed) #每次生成相同的结果
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True  # 开启CUDA的确定性算法

    mlops.pre_setup(args)  # 机器学习运营框架的prev_setup()函数，用于在模型训练或推理前执行一些预处理操作
...
from fedml.launch_simulation import run_simulation

from fedml.launch_cross_silo_horizontal import run_cross_silo_server
from fedml.launch_cross_silo_horizontal import run_cross_silo_client

from fedml.launch_cross_silo_hi import run_hierarchical_cross_silo_server
from fedml.launch_cross_silo_hi import run_hierarchical_cross_silo_client

from fedml.launch_cross_device import run_mnn_server
